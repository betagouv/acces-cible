module Checks
  class AnalyzeAccessibilityPage < Check
    ...
    HEADERS_SCOPE = ["Déclaration d’accessibilité", "État de conformité", "Résultats des tests", "Établissement de cette déclaration d’accessibilité"]
    ...
    def find_audit_date(pattern)
      section_text = []
      section_text << page.text(between_headings: [:previous, "État de conformité"])

      HEADERS_SCOPE.each do |header_scope|
        section_text << page.text(between_headings: [header_scope, :next])
      end

      section_text = section_text.compact.join(" ")
      strict_in_section = section_text.scan(pattern).presence
      date = extract_date(strict_in_section)

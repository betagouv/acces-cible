...
RSpec.describe Site do
  ...
  describe "#actual_current_audit" do
    ...
    context "when initial audit is uncompleted but other audits are completed" do
  ...
  describe "#set_current_audit!" do
    ...
    context "when there are multiple audits" do
      ...
      it "marks the newest completed audit as current" do
        ...
        newest_audit = create(:audit, :completed, site:, completed_at: 1.day.ago)
    ...
    context "when there are uncompleted audits only" do
    ...
    context "when there are both completed and uncompleted audits" do
      let!(:newest_completed_audit) { create(:audit, :completed, site:, completed_at: 2.days.ago) }
      let!(:newest_uncompleted_audit) { create(:audit, site:, completed_at: nil, created_at: 1.day.from_now) }

      it "marks the newest completed audit as current" do
        expect { site.set_current_audit! }.to change { newest_completed_audit.reload.current }.from(false).to(true)
        expect(newest_uncompleted_audit.reload.current).to be false
      end
    end
  end
end

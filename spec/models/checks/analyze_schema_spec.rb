...
RSpec.describe Checks::AnalyzeSchema do
  ...
  describe "#extract_valid_years" do
    {
      "Schéma pluriannuel d'accessibilité" => [],
      "Schéma pluriannuel d'accessibilité #{Date.current.year - 10}" => [],
      "uploads/#{Date.current.year}/schema-pluri-annuel-#{Date.current.year}02.pdf" => [Date.current.year],
      "SCHEMA PLURIANNUEL D'ACCESSIBILITE NUMERIQUE #{Date.current.year - 1}-#{Date.current.year}" => [Date.current.year - 1, Date.current.year],
      "SCHEMA PLURIANNUEL D'ACCESSIBILITE NUMERIQUE #{Date.current.year}-#{Date.current.year - 1}" => [Date.current.year - 1, Date.current.year],

  ...
  describe ".match?" do
    context "with default options" do
      it "matches identical strings" do
        expect(described_class.match?("hello", "hello")).to be true
      end

      it "does not match different strings" do
        expect(described_class.match?("hello", "world")).to be false
      end

      it "is case sensitive by default" do
        expect(described_class.match?("Hello", "hello")).to be false
      end

      it "handles nil values" do
        expect(described_class.match?(nil, "hello")).to be false
        expect(described_class.match?("hello", nil)).to be false
        expect(described_class.match?(nil, nil)).to be false
      end
    end
    ...
    context "with case insensitive option" do
      ...
      it "matches same strings with different case" do
        expect(described_class.match?("Hello", "hello", options)).to be true
        expect(described_class.match?("WORLD", "world", options)).to be true
      end

      it "still doesn't match different strings" do
        expect(described_class.match?("hello", "world", options)).to be false
      end
    end
    ...
    context "with partial matching" do
      ...
      it "matches when one string contains the other" do
        expect(described_class.match?("hello world", "hello", options)).to be true
        expect(described_class.match?("hello", "hello world", options)).to be true
      end

      it "is still case sensitive by default" do
        expect(described_class.match?("Hello world", "hello", options)).to be false
      end
    end
    ...
    context "with fuzzy matching" do
      ...
      it "matches strings with minor differences" do
        expect(described_class.match?("hello", "helo", options)).to be true
        expect(described_class.match?("address", "adress", options)).to be true
      end

      it "doesn't match strings with major differences" do
        expect(described_class.match?("hello", "goodbye", options)).to be false
      end
      ...
      context "when fuzzy threshold is zero" do
        ...
        it "raises an ArgumentError" do
          expect { described_class.match?("a", "b", options) }.to raise_error(ArgumentError)
        end
      end
    end
    ...
    context "with combined options" do
      ...
      it "matches partial case-insensitive fuzzy strings" do
        expect(described_class.match?("Hello world", "hello werlt", options)).to be true
        expect(described_class.match?("Contact Information", "contact info", options)).to be true
      end
    end
  end

...
RSpec.describe ProcessSiteUploadJob do
  ...
  describe "#perform" do
    it "enqueues one job for the batch" do
      expect { described_class.perform_now(sites_data, team.id, user.id, audit_batch.id) }
        .to have_enqueued_job(ProcessAuditBatchCreationJob).with(sites_data, team.id, user.id, audit_batch.id).exactly(:once)
    end
